...
export default function HomeScreen() {
  ...
  const [goalSidebarVisible, setGoalSidebarVisible] = useState(false);
  ...
  // Sync status
  useEffect(() => {
    if (isTranscriptLoading || isTranscriptFetching) {
      setStatus("loading");
    } else if (transcript) {
      setStatus("idle");
    }
  ...
  // Daily progress
  async function savePracticeHistory(sentence: string, score: number) {
    try {
      const newEntry = { sentence, score, date: new Date().toISOString() };
  ...
  async function incrementDailyCount() {
    try {
      ...
      obj.count += 1;
      await AsyncStorage.setItem("dailyProgress", JSON.stringify(obj));
      setDailyCount(obj.count);
    } catch (e) {
  ...
  // Progress sidebar toggle (daily progress only)
  ...
  const cardRotate = cardRotateAnim.interpolate({ inputRange: [0, 1], outputRange: ["-1deg", "1deg"] });
  const iconTranslateY = iconFloatAnim.interpolate({ inputRange: [0, 1], outputRange: [0, -2] });
  ...
  return (
    <SafeAreaView style={tw`flex-1 bg-yellow-50`}>
      ...
      {/* Progress Sidebar (daily progress only) */}
      <Animated.View
        style={[
      ...
      >
        <View style={tw`flex-1 p-4 pt-20 bg-yellow-50`}>
          <View style={tw`flex-row justify-between items-center mb-5`}>
            <Text style={tw`text-3xl font-bold text-amber-600`}>Daily Progress</Text>
            <Pressable onPress={() => setGoalSidebarVisible(true)}>
            <Settings size={30} color="#f59e0b" />
          </Pressable>
          </View>
          <DailyGoal count={dailyCount} goal={dailyGoal} onOpenSidebar={() => setGoalSidebarVisible(true)} />
        </View>
      </Animated.View>

      {/* Goal Sidebar (settings) */}
      {goalSidebarVisible && (
        <GoalSidebar
          visible={goalSidebarVisible}
          onClose={() => setGoalSidebarVisible(false)}
        />
      )}
      ...
      <Animated.View
        style={[
          tw`pt-8 pb-6 px-6 flex-row items-center justify-between`,  // Decreased top padding
          { opacity: fadeAnim, transform: [{ translateY: headerSlideAnim }] },
        ]}
      >
        <Pressable onPress={toggleProgressSidebar}>
          <Menu size={28} color="#f59e0b" />
        </Pressable>

        <LinearGradient
          colors={["#f2e7deff", "#F59E0B", "#f2e7deff"]}
          start={[0, 0]}
          end={[1, 1]}
          style={tw`px-4 py-1 rounded-full shadow-md`}
        >
          <Text
            style={{
      ...
      <ScrollView style={tw`flex-1`} contentContainerStyle={tw`px-6 pb-8`}>
        <Text style={[tw`text-base font-bold text-center mt-1 mb-3`, { color: colors.text }]}>
        ...
        >
          <Animated.View
            style={[
              ...
              { backgroundColor: colors.accent, opacity: 0.05, transform: [{ translateX: shimmerTranslate }] },
            ]}
          />
        ...
        <Animated.View
          style={[
            ...
            { opacity: fadeAnim, transform: [{ scale: buttonScaleAnim }] },
          ]}
        >
          ...
          <Animated.View
            style={{
              transform: [{ scale: recorderState.isRecording ? recordPulseAnim : buttonPulseAnim }],
              flex: 1,
            }}
          >
            <Pressable
            ...
            >
              <View style={tw`items-center`}>
                ...
                <Text
                  style={[
                    tw`text-sm font-bold uppercase tracking-wide`,
                    recorderState.isRecording || status === "analysing" ? tw`text-white` : tw`text-slate-700`,
                  ]}
                >
                  {recorderState.isRecording
                    ? "Stop"
      ...
      {mutation.isPending && (
        <View
          style={tw`flex-row items-center justify-center gap-2 p-8 bg-white/80 absolute bottom-0 left-0 right-0`}
        >
